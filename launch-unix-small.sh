...
if [ "$1" == "--reinstall-device" ]; then
  ...
  $sdkPath/tools/bin/avdmanager delete avd --name "SmallScreen"
  $sdkPath/tools/bin/avdmanager create avd --name "SmallScreen" --package "system-images;android-$version;google_apis_playstore;x86_64" --device "3.7in WVGA (Nexus One)"
fi

export ANDROID_SDK_ROOT=$sdkPath

$sdkPath/tools/bin/sdkmanager --update
exec $sdkPath/emulator/emulator @SmallScreen > /dev/null &
